...
class SimulationEngine:
    ...
    def run(self, incoming_rps: int = 1000) -> SimulationResult:
        ...
        # nodes with highest outdegree
        if not entry_points:
            entry_points = [
                max(self.graph.nodes(), key=lambda n: self.graph.out_degree(n))
            ]
        
        #distribute load across entry points
        ...
        node_bottlenecks = self._find_bottleneck_nodes()
        edge_overloads = self._find_overloaded_edges()
        cascade_risks = self._find_cascade_risks(node_bottlenecks)
        ...
        result = SimulationResult(
            ...
            node_loads=self._build_node_load_data(),
            edge_loads=self._build_edge_load_data(),
    ...
    def _generate_warnings(self, result: SimulationResult) -> List[str]:
        ...
        if result.bottleneck_nodes:
            warnings.append(
                f"⚠️  Found {len(result.bottleneck_nodes)} bottleneck node(s): "
                f"{', '.join(result.bottleneck_nodes)}"
            )
 
        if result.overloaded_edges:
            warnings.append(
                f"⚠️  Found {len(result.overloaded_edges)} overloaded edge(s). "
                "Network saturation risk."
            )
 
        if result.cascade_risk_nodes:
            warnings.append(
                f"⚠️  Found {len(result.cascade_risk_nodes)} cascade failure risk node(s). "
